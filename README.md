...
| 操作 | 气消耗 | 说明 |
|------|--------|------|
| 买入 | 动态 | 取决于卡牌评分；杠杆会额外锁定保证金 |
| 卖出 | 固定 -4 气 | 先返还该牌锁定保证金（受 80 气上限截断），再扣退出费 |
| 等待 | 0 气 | 下回合额外回复 10 气 |
| 持仓 | 每回合扣 | 评分越高，持仓成本越高 |
...
点击杠杆即使当前为 1.0x 也会预埋仓位；后续倍率按当前季内回合数升档。换季后所有杠杆仓位回到 1.0x，再随新季进度升档。杠杆放大收益，也增加持仓成本；气归零时触发强制平仓！
