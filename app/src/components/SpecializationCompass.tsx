  ...
  return (
    <div className="flex items-center justify-between gap-1 rounded-lg border border-wood-light/60 bg-[#faf6ee] px-2.5 py-0.5 shadow-2xs">
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        {grandCycles > 0 && (
          <span className="text-[10px] font-bold font-serif text-gold bg-gold/15 px-1 py-0.2 rounded border border-gold/40">
            混元×{grandCycles}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-1 min-w-0">
        {ELEMENT_METAS.map((meta) => {
          ...
          return (
            <div
              key={meta.key}
              className={`flex items-center gap-0.5 px-1.5 py-0.2 rounded border text-[11px] font-serif transition-all ${
                isBoosted
            ...
            >
              ...
              <span className="tabular-nums font-mono text-[10px] leading-none">
                {isBoosted ? `+${pct}%` : '1.0x'}
              </span>
            </div>
          );
