...
export function TopPanel() {
  ...
  return (
    <div className="flex flex-col bg-[#faf6ee] border-b border-wood-light">
      {/* 行 1：主要状态与操作栏 */}
      <div className="flex items-center justify-between gap-1.5 px-3 py-1.5 max-md:py-1">
        {/* 左侧：天时 + 年岁轮次 + 机缘 */}
        <div className="flex items-center gap-1.5 min-w-0 flex-wrap">
          <h1 className={`text-base font-bold font-serif ${seasonTheme.text} leading-none whitespace-nowrap`}>
            {/* key 变化触发切换动画，提示回合推进 */}
            <span key={currentRound} className="inline-block" style={roundAnimStyle}>
              {seasonDisplay(season)} · 天时
            </span>
          </h1>
          <span className="text-[11px] font-bold text-amber-900 bg-amber-100/90 px-1.5 py-0.5 rounded-full border border-amber-300 shadow-xs tabular-nums whitespace-nowrap">
            第 {year} 年 · {turn}/20 轮
          </span>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {gameState === 'player_action' && (
            <div className="flex items-center gap-1">
              <button
                ...
                className="px-1.5 py-0.5 rounded bg-white border border-wood-mid text-wood-dark text-[10px] font-bold font-serif hover:bg-wood-light/20 transition-all cursor-pointer"
                aria-label="打开交易看板"
              >
                行迹
              </button>
          ...
          <div data-score-panel className="score-panel relative shrink-0 rounded-lg border border-gold/40 bg-gold/10 px-2 py-0.5 text-right flex items-baseline gap-0.5">
            <span className="text-base font-black text-gold tabular-nums font-mono leading-none">
              {score.toFixed(1)}
            </span>
            ...
            {scoreDelta && (
              <div className={`mt-0.5 whitespace-nowrap text-[9px] font-bold leading-none tabular-nums absolute right-0 top-full ${scoreDelta.delta >= 0 ? 'text-qi-full' : 'text-qi-critical'}`}>
      ...
      {/* 行 2：季内回合与天劫道基大考进度条（独占一行，绝不挤压折行） */}
      <div className="flex items-center justify-between gap-2 px-3 py-1 bg-stone-900/[0.02] border-t border-wood-light/40 text-xs">
        <span className="font-medium text-ink text-[11px] whitespace-nowrap">
          季内第 <strong className="font-mono font-bold text-ink">{roundInSeason}</strong> 回合
        </span>
        <div className="flex items-center gap-1.5 flex-1 max-w-[220px] justify-end" title={`天劫门槛目标: ${score.toFixed(0)} / ${quota}`}>
          <span className="text-[10px] text-wood-dark font-medium whitespace-nowrap">天劫道基:</span>
          <div className="h-1.5 flex-1 rounded-full bg-wood-light/60 overflow-hidden min-w-[60px]">
            <div
      ...
      {branchRollDeltas && (
        <div
          className="grid grid-cols-12 gap-0.5 px-3 pb-0.5 pt-0.5 text-center border-t border-wood-light/50"
          aria-label="本季地支偏移"
      ...
      {activeSectIds.length > 0 && (
        <div
          className="flex items-center justify-between px-3 py-0.5 bg-stone-900/5 text-[10px] border-t border-wood-light/40"
        ...
        >
          <div className="flex items-center gap-1.5 font-serif text-wood-dark">
            ...
            {activeSectIds.map((sid: string) => {
              ...
              return (
                <span
                  key={sid}
                  className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded border text-[9px] font-sans ${
                    isUrgent
          ...
          <span className="text-[9px] text-amber-700/80 whitespace-nowrap">⚠️ 仅查丹田明牌 · 免查地脉暗牌</span>
        </div>
      )}
    </div>
  );
