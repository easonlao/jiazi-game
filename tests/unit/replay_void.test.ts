...
describe('函数层 V5 路径（共享核心；Edge Function 无法在 vitest 中导入 Deno/esm.sh 模块）', () => {
  it('版本门控：V4/V5/V6/V7/V8 接受，未注册版本（含 V1-V3、NaN）拒绝', () => {
    ...
    expect(getReplayRulesByVersion(11)).toBe(V11_MACRO_ROGUELIKE_REPLAY_RULES);
    ...
    expect(SUPPORTED_REPLAY_RULES.map((rules) => rules.rulesVersion)).toEqual([4, 5, 6, 7, 8, 9, 10, 11]);
    // V11 是生产默认，历史版本仍在注册表中用于历史会话的冻结解释。
