...
describe('TurnManager - 爆仓强平边界与控制流测试', () => {
  ...
  it('用例 2：单张牌强平分数结算与气扣除绕过校验', async () => {
    ...
    // 验证分数（2026-08-05 用户确认：强平不卖出、无收益，直接反噬罚分）：
    // 1. 持仓结算得分：1.2 * 4.0 * 2.5 = 12
    // 2. 强平无卖出收益（不再 8 折结算）
    // 3. 爆仓扣分：2.5 * |4.0| * 3 = 30
    // 最终分：100 + 12 - 30 = 82
    expect(tm.getScore()).toBeCloseTo(82.0, 1);
    expect(tm.getHand()[0]).toBeNull();
  });
  ...
  it('用例 3：多张杠杆卡牌循环强平校验', async () => {
    ...
    // 选「评分最高」的杠杆牌强平：卡牌 1 评分 4.0 > 卡牌 2 评分 3.0 → 强平卡牌 1（索引 0）
    // （2026-08-05 用户确认：选牌从随机改为评分最高，无需 Math.random mock，但保留以兼容旧随机源注入）

    // 推进回合
    tm.executeWait();

    // 验证仅第一张卡牌（评分最高 4.0）被强平出售，第二张卡牌被保留
    expect(tm.getHand()[0]).toBeNull();
    expect(tm.getHand()[1]).not.toBeNull();
    expect(tm.getHand()[1]!.card.id).toBe(2);

    // 验证分数（2026-08-05：强平不卖出、无收益，直接反噬罚分）：
    // 卡牌 1：持仓结算得分 (1.2 * 4 * 2.5 = 12)，强平无卖出收益，爆仓扣分 (2.5*|4|*3=30)
    // 卡牌 2：持仓结算得分 (1.2 * 3 * 2.5 = 9)，未被强平
    // 最终分：0 + 12 - 30 + 9 = -9（分数可以为负，不截断）
    expect(tm.getScore()).toBeCloseTo(-9, 1);
  });
