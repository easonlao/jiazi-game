...
describe('TurnManager - 爆仓强平边界与控制流测试', () => {
  ...
  it('用例 2：单张牌强平分数结算与气扣除绕过校验', async () => {
    ...
    // 3. 爆仓扣分：2.5 * |4.0| * 3 = 30
    // 最终分：100 + 12 + 8 - 30 = 90
    expect(tm.getScore()).toBeCloseTo(90.0, 1);
    expect(tm.getHand()[0]).toBeNull();
  });
  ...
  it('用例 3：多张杠杆卡牌循环强平校验', async () => {
    ...
    // 验证分数：
    // 卡牌 1：持仓结算得分 (1.2 * 4 * 2.5 = 12)，强平卖出得分 8（8折后），爆仓扣分 (2.5*|4|*3=30)
    // 卡牌 2：持仓结算得分 (1.2 * 3 * 2.5 = 9)，未被卖出
    // 最终分：0 + 12 + 8 - 30 + 9 = -1（分数可以为负，不截断）
    expect(tm.getScore()).toBeCloseTo(-1, 1);
